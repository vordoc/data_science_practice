...
Некоторые сторонние библиотеки имеют совместимые с pandas методы чтения 
для получения доступа к данным из различных онлайн-источников, таких как Quandl1 и Stooq2. 
Самая популярная подобная библиотека — pandas-datareader. 
На момент написания книги эта библиотека включала 70 методов для загрузки данных из определенного источника и их 
преобразования в pandas DataFrame. 
Многие методы библиотеки являются обертками для финансовых API, позволяя легко получать финансовые данные 
в формате pandas.
Установка pandas-datareader
Описание методов чтения библиотеки можно найти в документации pandas-datareader3. 
Кроме того, можно вывести на экран список доступных методов с помощью функции dir()

Метод get_data_stooq() получает данные со Stooq, бесплатного сайта, который  предоставляет информацию по ряду 
рыночных индексов. В качестве первого параметра передаем тикер нужного рыночного индекса. 
Доступные опции можно найти на сайте https://stooq.com/t/
Посмотри данные по S&P500 c 3 по 10 января 2024 г"""

spx_index = pdr.get_data_stooq('^SPX', '2024-01-03', '2024-01-10')
print(spx_index)
